...
export function usePlayerMpvLifecycle(options: Options) {
  const { stateRef, mpvInitializedRef, playerUsesTorrentRef, inNativePlayerRef, artworkPrefetchRef, pendingArtworkRef, isCancelled, stopTorrent, debugLog, setPlayerTitle, setPlayerUrl, setPlayerUsesTorrent, setPlayerSubtitleUrl, setPlayerSubtitleWarning } = options;
  ...
  return useCallback(async (
    ...
    if (isCancelled(generation)) return;
    await embeddedMpvApplyPreferences({ ...appPrefs(stateRef.current), isTorrentPlayback: usesTorrent, isAnimePlayback: !!isAnimePlayback }).catch(() => undefined);
    await embeddedMpvSetTitle(title?.contentTitle, title?.episodeLine).catch(() => undefined);
    if (isCancelled(generation)) return;
    if (!inNativePlayerRef.current) {
      if (artworkPrefetchRef.current) await Promise.race([artworkPrefetchRef.current, new Promise<void>((resolve) => setTimeout(resolve, 2000))]);
      if (isCancelled(generation)) return;
      inNativePlayerRef.current = true;
      await embeddedMpvShowLoading(title?.contentTitle ?? 'Fluxa', title?.episodeLine);
      void embeddedMpvSetLoadingArtwork(title?.contentTitle ?? 'Fluxa', title?.episodeLine, pendingArtworkRef.current?.background, pendingArtworkRef.current?.logo).catch(() => undefined);
      if (isCancelled(generation)) return;
    ...
    if (usesTorrent) {
      ...
      for (const sibling of siblingSubtitles) {
        ...
        subtitles.push({ url: sibling.url, label: sibling.title ?? sibling.language ?? '', lang: sibling.language, addonName: 'Torrent' });
      }
    }
    if (isCancelled(generation)) return;
