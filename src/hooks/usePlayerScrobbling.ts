...
export function usePlayerScrobbling(options: Options) {
  ...
  useEffect(() => {
    ...
    listen<boolean>('native-player-pause-changed', (event) => {
      void handlePauseChanged(event.payload);
    })
      .then((fn) => {
        if (cancelled) fn();
        else unlisten = fn;
      ...
      .catch(() => undefined);
    void embeddedMpvStatus()
      .then((status) => {
        if (!cancelled && status) void handlePauseChanged(status.pause === 'yes');
      })
      .catch(() => undefined);
    return () => {
      cancelled = true;
