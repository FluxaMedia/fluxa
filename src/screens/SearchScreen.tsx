...
function groupCategoriesByAddon(categories: HomeCategory[]): HomeCategory[] {
  ...
  return order.flatMap((name) => groups.get(name)!.sort((a, b) => (a.type === 'series' ? 1 : 0) - (b.type === 'series' ? 1 : 0)));
}
...
export const SearchScreen = React.memo(
  function SearchScreen({ state, onDispatch, onNavigateDetail, query, onQueryChange, onBack }: Props) {
    ...
    useEffect(
      () =>
        addSearchPartialHandler((q, source: PartialSearchSource, items) => {
    ...
    useEffect(() => {
      let active = true;
      void coreInvoke<typeof screenPlan>(
        'searchScreenPlan',
        JSON.stringify({
          query,
    ...
    useEffect(() => {
      ...
      if (screenPlan.shouldDispatch)
        onDispatch(JSON.stringify({ type: 'searchRequested', query: screenPlan.query, language: getLanguage() }));
    ...
    return (
      <div className="search-screen" style={styles.screen}>
        <div className="search-content" style={styles.content}>
          ...
          {!query && recentSearches.length > 0 && (
            <>
              <div style={styles.sectionHeaderRow}>
                ...
                <button style={styles.clearRecentBtn} onClick={handleClearRecent}>
                  {t('common.clear')}
                </button>
              </div>
          ...
          {isLoading && orderedPartialCategories.length === 0 && <LoadingShelves />}
