...
export function useCalendarArtwork(items: CalendarItem[]) {
  ...
  useEffect(() => {
    ...
    void Promise.all([...itemsBySeries].map(async ([id, seriesItems]) => {
      let meta: { poster?: string; videos?: Video[] } | null = null;
      try {
        meta = await fetchMetaDetail({ id, contentType: seriesItems[0]?.metaType === 'movie' ? 'movie' : 'series' }) as { poster?: string; videos?: Video[] } | null;
      } catch {
        return [];
      }
      return seriesItems.map((item) => {
        ...
        const matchedEpisode = meta?.videos?.find((video) => season != null && episode != null ? video.season === season && (video.episode ?? video.number) === episode : !!item.dateIso && !!video.released && localDateKeyFromIso(video.released) === localDateKeyFromIso(item.dateIso));
        const episodePoster = matchedEpisode?.thumbnail;
        return { key: calendarArtworkKey(item), artwork: episodePoster ?? meta?.poster, episode: matchedEpisode ? { seasonNumber: matchedEpisode.season, episodeNumber: matchedEpisode.episode ?? matchedEpisode.number, episodeTitle: matchedEpisode.title ?? matchedEpisode.name, episodePoster } : null };
      });
    })).then((entries) => {
      if (!active) return;
