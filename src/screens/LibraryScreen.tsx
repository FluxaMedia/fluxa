...
import {
  loadProviderLibraries,
  PROVIDER_LIBRARIES_CHANGED,
  type LibraryProvider,
  type ProviderLibrarySnapshot,
} from '../core/providerLibraries';
...
export const LibraryScreen = React.memo(
  function LibraryScreen({ state, onDispatch, onNavigateDetail, onBack, activeProfile, onProfileUpdated, onPlayLocal }: Props) {
    ...
    const [sortBy, setSortBy] = useState<'recent' | 'title' | 'rating'>(
      () => (getViewPrefs().librarySort as 'recent' | 'title' | 'rating') ?? 'recent',
    );
    ...
    const changeTab = (v: Tab) => {
      setTab(v);
      setViewPref('libraryTab', v);
    };
    const changeSort = (v: 'recent' | 'title' | 'rating') => {
      setSortBy(v);
      setViewPref('librarySort', v);
    };
    const changeLibrarySource = (v: LibrarySource) => {
      setLibrarySource(v);
      setViewPref('librarySource', v);
    };
    ...
    useEffect(() => {
      ...
      debugLog(
        `LibraryScreen: settings-sync effect source=${source} lastApplied=${lastSettingsLibrarySourceRef.current} currentLibrarySource=${librarySource}`,
      );
    ...
    const watchlist = (
      librarySource === 'local' ? (library.lastWrite?.watchlist ?? library.watchlist ?? []) : (selectedProviderLibrary?.watchlist ?? [])
    ) as LibraryItem[];
    const watching = (
      librarySource === 'local'
        ? (library.lastWrite?.continueWatching ?? library.continueWatching ?? [])
        : (selectedProviderLibrary?.watching ?? [])
    ) as LibraryItem[];
    const rawCompleted = (
      librarySource === 'local' ? (library.lastWrite?.completed ?? library.completed ?? []) : (selectedProviderLibrary?.completed ?? [])
    ) as LibraryItem[];
    const rawDropped = (
      librarySource === 'local' ? (library.lastWrite?.dropped ?? library.dropped ?? []) : (selectedProviderLibrary?.dropped ?? [])
    ) as LibraryItem[];
    const favorites = (
      librarySource === 'local' ? (library.lastWrite?.favorites ?? library.favorites ?? []) : (selectedProviderLibrary?.favorites ?? [])
    ) as LibraryItem[];

    const watchlistLabelKey =
      librarySource === 'trakt' || librarySource === 'nuvio' || librarySource === 'stremio'
        ? 'library.watchlist'
    ...
    useEffect(() => {
      let active = true;
      void coreInvoke<typeof viewPlan>(
        'libraryViewPlan',
        JSON.stringify({
          watchlist,
          watching,
          ...
          dropped: rawDropped,
          favorites,
          progress: library.lastWrite?.progress ?? {},
          tab,
          query,
          sortBy,
        }),
      ).then((plan) => {
        if (active && plan) setViewPlan(plan);
      });
      return () => {
        active = false;
      };
    ...
    const subtitle =
      tab === 'watchlist'
        ? t('auto.movies_and_shows_you_saved_to_watch_later')
        : tab === 'watching'
          ? t('library.subtitle_watching')
          : tab === 'completed'
            ? t('library.subtitle_completed')
            : tab === 'dropped'
              ? t('library.subtitle_dropped')
              : tab === 'airing'
                ? t('library.subtitle_airing')
                : tab === 'rated'
                  ? t('library.subtitle_rated')
                  : tab === 'history'
                    ? t('library.subtitle_history')
                    : t('library.subtitle_collections');
    const libraryTitle = librarySource === 'local' ? t('auto.my_library_a6c93797') : t(`library.source_${librarySource}`);
      ...
      <div className="library-screen" style={styles.screen}>
        ...
        <div className="library-tabs" style={styles.tabRow}>
          ...
          <TabChip active={tab === 'watching'} onClick={() => changeTab('watching')}>
            {t('library.watching')}
            {watching.length > 0 ? ` (${watching.length})` : ''}
          </TabChip>
          {completedTabEnabled && (
          ...
          <TabChip active={tab === 'dropped'} onClick={() => changeTab('dropped')}>
            {t('library.dropped')}
            {dropped.length > 0 ? ` (${dropped.length})` : ''}
          </TabChip>
          ...
          <TabChip active={tab === 'airing'} onClick={() => changeTab('airing')}>
            {t('library.smart_airing')}
            {smartLists.airing.length > 0 ? ` (${smartLists.airing.length})` : ''}
          </TabChip>
          <TabChip active={tab === 'rated'} onClick={() => changeTab('rated')}>
            {t('library.smart_rated')}
            {smartLists.rated.length > 0 ? ` (${smartLists.rated.length})` : ''}
          </TabChip>
          <TabChip active={tab === 'history'} onClick={() => changeTab('history')}>
            {t('library.history')}
            {smartLists.history.length > 0 ? ` (${smartLists.history.length})` : ''}
          </TabChip>
          ...
          {tab !== 'collections' && !localMediaOpen && (
            <div className="library-controls" style={styles.controls}>
              ...
              <FilterDropdown
                value={
                  sortBy === 'recent' ? t('library.sort_recent') : sortBy === 'title' ? t('library.sort_title') : t('library.sort_rating')
                }
                options={[
              ...
              <button
                style={{
                  ...styles.bulkToggle,
                  background: bulkMode ? '#FFFFFF' : 'rgba(255,255,255,0.05)',
                  color: bulkMode ? '#000' : '#fff',
                }}
                onClick={() => {
                  setBulkMode((v) => !v);
                  clearSelection();
        ...
        {bulkMode && tab !== 'collections' && !localMediaOpen && (
          <div style={styles.bulkBar}>
            ...
            <div style={{ flex: 1 }} />
            <button style={styles.bulkBtn} disabled={selectedIds.size === 0} onClick={() => markSelectedWatched(true)}>
              {t('detail.mark_watched')}
            </button>
            <button style={styles.bulkBtn} disabled={selectedIds.size === 0} onClick={() => markSelectedWatched(false)}>
              {t('detail.mark_unwatched')}
            </button>
            <button style={styles.bulkBtn} disabled={selectedIds.size === 0} onClick={() => moveSelectedToStatus('completed')}>
              {t('library.mark_completed')}
            </button>
            <button style={styles.bulkBtn} disabled={selectedIds.size === 0} onClick={() => moveSelectedToStatus('dropped')}>
              {t('library.mark_dropped')}
            </button>
            {canRemoveFromCurrentList && (
              <button style={styles.bulkDangerBtn} disabled={selectedIds.size === 0} onClick={() => setConfirmBulkRemove(true)}>
                {t('common.remove')}
              </button>
            )}
            <button
        ...
        {localMediaOpen ? (
          <LocalMediaPanel posterPrefs={posterPrefs} onPlay={(stream, meta, episode) => onPlayLocal?.(stream, meta, episode)} />
        ) : tab === 'collections' ? (
          ...
          <div style={styles.empty}>
            <p style={styles.emptyTitle}>
              {tab === 'watchlist'
                ? t('library.your_list_empty')
                : tab === 'watching'
                  ? t('library.nothing_to_continue')
                  : tab === 'completed'
                    ? t('library.nothing_completed')
                    : tab === 'dropped'
            ...
            <p style={styles.emptyHint}>
              {tab === 'watchlist'
                ? t('library.add_titles_hint')
                : tab === 'watching'
                  ? t('library.start_watching_hint')
                  : tab === 'completed'
                    ? t('library.completed_hint')
                    : tab === 'dropped'
                      ? t('library.dropped_hint')
                      : tab === 'history'
                        ? t('library.history_empty_hint')
