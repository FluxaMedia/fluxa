...
export async function coreEffectiveMetadataFeedSelection(selectedKeys: string[], availableKeys: string[]): Promise<string[] | null> {
  return coreInvoke(
...
export async function coreDetectAnimePlayback(
  ...
  return (
    (await coreInvoke('detectAnimePlayback', JSON.stringify({ meta, episode, stream, addons }))) ?? {
      isAnime: false,
      confidence: 0,
      reasons: [],
    }
...
export async function coreAnilistMediaListStatus(totalEpisodes: number, progressEpisode: number): Promise<'COMPLETED' | 'CURRENT'> {
  return (
    (await coreInvoke<'COMPLETED' | 'CURRENT'>('anilistMediaListStatus', JSON.stringify({ totalEpisodes, progressEpisode }))) ?? 'CURRENT'
  );
}
