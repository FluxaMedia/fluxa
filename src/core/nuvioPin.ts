...
export async function nuvioPinCachePayload(profileIndex: number, salt: string, pin: string, profileUpdatedAt?: string): Promise<CachedPin | null> {
  const raw = await coreInvoke<string>('nuvioPinCachePayload', JSON.stringify({ profileIndex, salt, pin, profileUpdatedAt: profileUpdatedAt ?? '' }));
...
export async function nuvioVerifyCachedPin(profile: UserProfile, pin: string): Promise<OfflineResult> {
  ...
  const raw = await coreInvoke<string>('nuvioPinVerifyCached', JSON.stringify({
