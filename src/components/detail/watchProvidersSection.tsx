...
export const WatchProviderLogo = React.memo(function WatchProviderLogo({ name, logo, onClick }: { name: string; logo?: string; onClick: () => void }) {
  ...
  return (
    ...
    >
      {showImage ? (
        <img src={logo} alt={name} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} onError={() => setImageFailed(true)} />
      ) : (
