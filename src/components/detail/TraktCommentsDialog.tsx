...
function CommentAvatar({ name, url }: { name: string; url?: string }) {
  ...
  if (!imageUrl || failed) return <span aria-hidden="true" style={styles.avatarFallback}>{initials}</span>;
...
export function TraktCommentsDialog({ meta, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const request = await coreInvoke<{ resource: string; id: string; lookupType: string; wantType: string } | null>('traktCommentsRequest', JSON.stringify({ contentId: meta.id, contentType: meta.type }));
        if (!clientId || !request?.resource || !request.id || !request.lookupType || !request.wantType) throw new Error('comments unavailable');
        const headers = {
        ...
        const slug = await slugFromSearch(
  ...
  return (
    <div role="presentation" onClick={onClose} style={styles.backdrop}>
      <section role="dialog" aria-modal="true" aria-label={t('detail.trakt_comments')} onClick={(event) => event.stopPropagation()} style={styles.dialog}>
        <div style={styles.header}>
          <div>
            <h2 style={styles.title}>{t('detail.trakt_comments')}</h2>
            <p style={styles.subtitle}>{meta.name}</p>
          </div>
          <button type="button" aria-label={t('common.close')} title={t('common.close')} onClick={onClose} style={styles.closeButton}><X size={20} /></button>
        </div>
          ...
          {!loading && !error && comments.map((comment, index) => (
            <article key={comment.id ?? index} style={styles.comment}>
              <div style={styles.commentHeader}>
                <CommentAvatar name={comment.user?.name || comment.user?.username || t('detail.comments_anonymous')} url={comment.user?.images?.avatar?.full} />
                ...
                {typeof comment.likes === 'number' && <span style={styles.stats}><Heart size={14} fill="currentColor" />{t('detail.comments_likes', comment.likes)}</span>}
              </div>
              ...
              <div style={styles.commentMeta}>
                ...
                {comment.updated_at && comment.updated_at !== comment.created_at && <span>{t('detail.comments_edited', formatDate(comment.updated_at))}</span>}
                ...
                {typeof comment.user_stats?.rating === 'number' && <span>{t('detail.comments_profile_rating', comment.user_stats.rating)}</span>}
                {typeof comment.user_stats?.play_count === 'number' && <span>{t('detail.comments_plays', comment.user_stats.play_count)}</span>}
                {typeof comment.user_stats?.completed_count === 'number' && <span>{t('detail.comments_completed', comment.user_stats.completed_count)}</span>}
              </div>
              {comment.user && <details style={styles.profile}>
...
const styles: Record<string, React.CSSProperties> = {
  backdrop: { position: 'fixed', inset: 0, zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1.5rem', background: 'rgba(0,0,0,0.72)' },
  dialog: { width: 'min(42rem, 100%)', maxHeight: 'min(44rem, calc(100vh - 3rem))', display: 'flex', flexDirection: 'column', overflow: 'hidden', border: '0.0625rem solid rgba(255,255,255,0.16)', borderRadius: '1rem', background: '#171717', color: '#fff', boxShadow: '0 1.5rem 5rem rgba(0,0,0,0.55)' },
  header: { display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', padding: '1.25rem 1.25rem 1rem', borderBottom: '0.0625rem solid rgba(255,255,255,0.1)' },
  title: { margin: 0, fontSize: '1.125rem' },
  subtitle: { margin: '0.25rem 0 0', color: 'rgba(255,255,255,0.58)', fontSize: '0.875rem' },
  closeButton: { display: 'flex', alignItems: 'center', justifyContent: 'center', width: '2.25rem', height: '2.25rem', border: 0, borderRadius: '50%', background: 'rgba(255,255,255,0.1)', color: '#fff', cursor: 'pointer' },
  ...
  avatarFallback: { width: '2.25rem', height: '2.25rem', flexShrink: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', borderRadius: '50%', background: 'rgba(255,255,255,0.14)', color: 'rgba(255,255,255,0.82)', fontSize: '0.75rem', fontWeight: 700 },
  authorBlock: { display: 'flex', flexDirection: 'column', gap: '0.0625rem' },
  username: { color: 'rgba(255,255,255,0.55)', fontSize: '0.75rem' },
  review: { padding: '0.125rem 0.375rem', borderRadius: '0.25rem', background: 'rgba(255,255,255,0.13)', color: 'rgba(255,255,255,0.76)', fontSize: '0.6875rem' },
  badge: { padding: '0.125rem 0.375rem', borderRadius: '0.25rem', background: 'rgba(255,255,255,0.09)', color: 'rgba(255,255,255,0.68)', fontSize: '0.6875rem' },
  stats: { marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '0.25rem', color: '#FF6B81', fontSize: '0.75rem', fontWeight: 600 },
  body: { margin: '0.625rem 0 0', color: 'rgba(255,255,255,0.82)', lineHeight: 1.55, whiteSpace: 'pre-wrap' },
  commentMeta: { display: 'flex', flexWrap: 'wrap', gap: '0.375rem 0.75rem', marginTop: '0.625rem', color: 'rgba(255,255,255,0.52)', fontSize: '0.75rem' },
  ...
  profileGrid: { display: 'flex', flexWrap: 'wrap', gap: '0.375rem 0.75rem', padding: '0 0.75rem 0.75rem', color: 'rgba(255,255,255,0.62)', fontSize: '0.75rem' },
