...
export function ConfirmDialog({ title, body, confirmLabel, cancelLabel, destructive, onConfirm, onCancel }: {
  title: string;
  body: string;
  confirmLabel: string;
  cancelLabel: string;
    ...
    <div style={S.overlay} onClick={onCancel}>
      <div style={S.dialog} onClick={(e) => e.stopPropagation()}>
        ...
        <div style={S.actions}>
          ...
          <button onClick={onConfirm} style={{ ...S.confirmBtn, background: destructive ? '#c0392b' : '#FFFFFF', color: destructive ? '#fff' : '#000' }}>
            {confirmLabel}
          </button>
        </div>
      </div>
...
const S: Record<string, React.CSSProperties> = {
  ...
  dialog: { width: '20rem', borderRadius: '0.75rem', background: '#141414', border: '1px solid rgba(255,255,255,0.10)', padding: '1.5rem', textAlign: 'center', fontFamily: FONT },
  ...
  confirmBtn: { flex: 1, height: '2.75rem', borderRadius: '0.5rem', border: 'none', fontSize: '0.8125rem', fontWeight: 600, fontFamily: FONT, cursor: 'pointer', outline: 'none' },
};
