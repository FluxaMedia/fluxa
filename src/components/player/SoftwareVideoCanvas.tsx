...
export function SoftwareVideoCanvas({ statusRef, onFirstFrame }: { statusRef: RefObject<EmbeddedMpvStatus | null>; onFirstFrame?: () => void }) {
  ...
  useEffect(() => {
    ...
    const draw = async () => {
      ...
      try {
        const frame = await embeddedMpvRenderFrame(Math.max(320, Math.floor(viewportW * scale)), Math.max(180, Math.floor(viewportH * scale)));
        if (cancelled) return;
        ...
        if (context) {
          ...
          const hasRenderedVideo = status?.loaded && status.hasVideoTrack && status.voConfigured === 'yes' && status.framesRendered >= 2 && status.pausedForCache !== 'yes' && parseFloat(status.timePos ?? '0') > 0.15;
  ...
  return <canvas ref={canvasRef} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'contain', background: '#000', zIndex: 0, pointerEvents: 'none' }} />;
}
