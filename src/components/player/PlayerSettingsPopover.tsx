...
function SettingsToggleRow({ label, checked, onToggle }: { label: string; checked: boolean; onToggle: () => void }) {
  return (
    ...
    >
      ...
      <div
        style={{ flexShrink: 0, width: '2.75rem', height: '1.625rem', borderRadius: '62.4375rem', background: checked ? 'var(--primary-accent-color)' : 'rgba(255,255,255,0.14)', position: 'relative', transition: 'background 0.18s' }}
      >
        <div style={{ position: 'absolute', top: '0.1875rem', left: checked ? 21 : 3, width: '1.25rem', height: '1.25rem', borderRadius: '50%', background: checked ? '#000000' : 'rgba(255,255,255,0.80)', transition: 'left 0.18s' }} />
      </div>
    </div>
  );
}

export function PlayerSettingsPopover({ anchorRef, onClose, anime4kEnabled, onToggleAnime4k, stableVolumeEnabled, onToggleStableVolume }: PlayerSettingsPopoverProps) {
  return (
    <Popover open onClose={onClose} anchorRef={anchorRef} placement="top" width="15rem">
      ...
      <SettingsToggleRow label={t('player.stable_volume')} checked={stableVolumeEnabled} onToggle={() => onToggleStableVolume(!stableVolumeEnabled)} />
    </Popover>
  );
}
