...
const hdrBadgeStyle: CSSProperties = {
  display: 'inline-flex',
  alignItems: 'center',
...
export function PlayerHeader({
  ...
  return (
    ...
    >
      <button onClick={(event) => stopAndRun(event, onClose)} className="fluxa-ibtn" style={iconButtonStyle} title={t('player.back')}>
        <ChevronLeft size={22} />
      </button>
      ...
      {activeCastDeviceId && (
        <button
          onClick={(event) => stopAndRun(event, onToggleCastPause)}
          className="fluxa-ibtn"
          style={iconButtonStyle}
          title={castPaused ? t('player.play') : t('player.pause')}
        >
      ...
      <button
        ref={streamLinksButtonRef}
        onClick={(event) => stopAndRun(event, onOpenStreamLinks)}
        className="fluxa-ibtn"
        style={iconButtonStyle}
      ...
      <button
        ...
        onClick={(event) => stopAndRun(event, onToggleSettings)}
        className="fluxa-ibtn"
        style={iconButtonStyle}
      ...
      {canMarkSegments && (
        <button
          onClick={(event) => stopAndRun(event, onToggleSegmentMarker)}
          className="fluxa-ibtn"
          style={{ ...iconButtonStyle, color: showSegmentMarker ? 'var(--primary-accent-color)' : '#fff' }}
