...
data class MediaProbeReport(
    ...
    fun format(): String {
        ...
        lines += "Engine: $engine"
        lines += "Source: ${source.format()}"
        lines += "Container: ${container.format()}"
        ...
        if (quirks.isNotEmpty()) {
            lines += "Quirks: ${quirks.joinToString(", ") { it.format() }}"
        }
...
data class ProbeValue(
    val value: String?,
    val confidence: ProbeConfidence = ProbeConfidence.Unknown
) {
    fun format(): String {
        val text = value?.takeIf { it.isNotBlank() } ?: "unknown"
        return when (confidence) {
            ProbeConfidence.Verified -> text
            ProbeConfidence.Inferred -> "$text (inferred)"
            ProbeConfidence.Unknown -> text
        }
    }

    companion object {
        fun verified(value: String?): ProbeValue = ProbeValue(value?.takeIf { it.isNotBlank() }, ProbeConfidence.Verified)
        fun inferred(value: String?): ProbeValue = ProbeValue(value?.takeIf { it.isNotBlank() }, ProbeConfidence.Inferred)
        fun unknown(): ProbeValue = ProbeValue(null, ProbeConfidence.Unknown)
    }
}
...
data class VideoProbeTrack(
    ...
    fun format(): String = listOfNotNull(
        codec.value?.let { it },
        profile.value?.let { it },
        level.value?.let { it },
        pixelFormat.value?.let { it },
        resolution.value?.let { it },
        fps.value?.let { "${it} fps" },
        bitrate.value?.let { it },
        language?.takeIf { it.isNotBlank() }?.let { it.uppercase(Locale.ROOT) }
    ).joinToString(" / ")
}
...
data class AudioProbeTrack(
    ...
    fun format(): String = listOfNotNull(
        codec.value?.let { it },
        channels.value?.let { it },
        sampleRate.value?.let { it },
        bitrate.value?.let { it },
        language?.takeIf { it.isNotBlank() }?.let { it.uppercase(Locale.ROOT) },
