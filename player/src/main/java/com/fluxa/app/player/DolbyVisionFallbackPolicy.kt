...
object DolbyVisionFallbackPolicy {
    ...
    fun capabilities(context: Context): DolbyVisionCapabilities {
        ...
        val decoders = queryDvDecoderProfiles()
    ...
    private data class DvDecoderProfiles(
    ...
    private fun queryDvDecoderProfiles(): DvDecoderProfiles {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
            return DvDecoderProfiles(false, false, false, false)
        }
        return runCatching {
            var nativeP7 = false; var p5 = false; var p8 = false; var anyDv = false
            for (info in MediaCodecList(MediaCodecList.REGULAR_CODECS).codecInfos) {
                if (info.isEncoder) continue
                if (!info.supportedTypes.any { it.equals(MimeTypes.VIDEO_DOLBY_VISION, ignoreCase = true) }) continue
                anyDv = true
                val caps = runCatching { info.getCapabilitiesForType(MimeTypes.VIDEO_DOLBY_VISION) }.getOrNull() ?: continue
                for (pl in caps.profileLevels ?: continue) {
                    when (pl.profile) {
            ...
            DvDecoderProfiles(nativeP7, p5, p8, anyDv)
        }.getOrDefault(DvDecoderProfiles(false, false, false, false))
    }
}
