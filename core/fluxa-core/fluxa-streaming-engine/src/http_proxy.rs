...
async fn resolve_public(url: &url::Url) -> Result<SocketAddr, Response> {
    let host = url
        .host_str()
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "url has no host").into_response())?;
    let port = url
        .port_or_known_default()
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "url has no port").into_response())?;
    let mut addresses = tokio::net::lookup_host((host, port))
        .await
        .map_err(|_| (StatusCode::BAD_REQUEST, "host cannot be resolved").into_response())?;
    addresses
        .find(|address| allowed_ip(address.ip()))
        .ok_or_else(|| (StatusCode::BAD_REQUEST, "private hosts are not allowed").into_response())
}
...
fn requested_headers(raw: Option<&str>) -> HeaderMap {
    ...
    for (name, value) in values {
        ...
        if name == header::REFERER || name == header::USER_AGENT {
...
pub async fn handle_proxy(Query(query): Query<ProxyQuery>, request: Request) -> Response {
    ...
    let address = match resolve_public(&url).await {
        Ok(address) => address,
    ...
    if let Some(value) = content_type {
        output.headers_mut().insert(header::CONTENT_TYPE, value);
    }
...
pub fn router() -> Router {
    Router::new().route("/proxy", get(handle_proxy))
}
