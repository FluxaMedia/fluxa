...
class PluginsStore(
    ...
    suspend fun dispatch(action: PluginsAction) {
        when (action) {
            ...
            is PluginsAction.CloudstreamRepoAdded -> dataSource.addCloudstreamRepository(action.url, action.publisherPublicKey)
            is PluginsAction.CloudstreamAutomaticUpdatesChanged -> dataSource.setCloudstreamAutomaticUpdatesEnabled(action.enabled)
