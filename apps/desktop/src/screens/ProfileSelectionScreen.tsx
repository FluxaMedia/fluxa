...
export function ProfileSelectionScreen({ onProfileSelected, onProfilesChanged }: Props) {
  ...
  const handleSelect = async (profile: UserProfile) => {
    if (profile.pinHash || profile.nuvioPinEnabled) {
    ...
    if (deleted?.nuvioAccessToken && deleted.nuvioUserId && deleted.nuvioProfileIndex != null) {
      void (async () => {
        try {
          ...
          const remoteProfiles = updated.filter(
            (profile) => profile.nuvioUserId === freshProfile.nuvioUserId && profile.nuvioProfileIndex != null,
          );
  ...
  const requestEdit = (profile: UserProfile) => {
    if (profile.pinHash) {
      setPinGate({ profile, action: 'edit' });
      return;
    }
    handleEdit(profile);
  };
  ...
  const requestDelete = (profile: UserProfile) => {
    if (profile.pinHash) {
      setPinGate({ profile, action: 'delete' });
      return;
    }
  ...
  return (
    <div
      style={{
        ...S.root,
        ...(pickerSettings.backgroundUrl
          ? {
              backgroundImage: `linear-gradient(rgba(12,12,12,0.78), rgba(12,12,12,0.88)), url(${pickerSettings.backgroundUrl})`,
    ...
    >
      ...
      <main style={showForm || isSettings ? S.main : S.mainSelect}>
        ...
        {isSettings && <ProfilePickerSettingsView settings={pickerSettings} onSaved={setPickerSettings} onBack={() => setMode('select')} />}
      </main>
...
function AddProfileCard({ onClick }: { onClick: () => void }) {
  ...
  return (
    <div style={S.profileCard}>
      <button onClick={onClick} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} style={S.profileSelectButton}>
        <div
          style={{
            ...S.addCircle,
