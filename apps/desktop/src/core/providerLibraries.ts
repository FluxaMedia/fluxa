...
export async function loadProviderLibraries(profileKey?: string): Promise<ProviderLibraries> {
      ...
      tasks.push(
        (async () => {
          ...
          if (result && typeof result === 'object' && 'snapshot' in result)
            remote.trakt = (result as { snapshot: ProviderLibrarySnapshot }).snapshot;
        })().catch(() => undefined),
      );
    }
    if (profile.simklAccessToken) {
      tasks.push(
        (async () => {
          ...
          if (result && typeof result === 'object' && 'snapshot' in result)
            remote.simkl = (result as { snapshot: ProviderLibrarySnapshot }).snapshot;
        })().catch(() => undefined),
      );
    }
    if (profile.anilistAccessToken) {
      tasks.push(
        (async () => {
          ...
          if (result && typeof result === 'object' && 'snapshot' in result)
            remote.anilist = (result as { snapshot: ProviderLibrarySnapshot }).snapshot;
        })().catch(() => undefined),
      );
    }
    if (profile.stremioAuthKey) {
      tasks.push(
        (async () => {
          ...
          if (result && typeof result === 'object' && 'snapshot' in result)
            remote.stremio = (result as { snapshot: ProviderLibrarySnapshot }).snapshot;
        })().catch(() => undefined),
      );
    }
    if (profile.nuvioAccessToken) {
      tasks.push(
        (async () => {
          ...
          remote.nuvio = {
            watchlist: watchlist as Record<string, unknown>[],
            watching: watching as Record<string, unknown>[],
            completed: [],
            dropped: [],
            favorites: [],
          };
  ...
  return Object.fromEntries(
    Object.entries(libraries).map(([provider, snapshot]) => [
      provider,
      {
...
export async function saveProviderLibrary(
