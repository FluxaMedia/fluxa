...
export function usePlayerCasting({
  ...
  useEffect(() => {
    ...
    return () => {
      if (activeCastDeviceId) castDisconnect();
    };
  }, [activeCastDeviceId]);
  ...
  useEffect(() => {
    openCastPopoverRef.current = openCastPopover;
  }, [openCastPopover]);
  ...
  const selectCastDevice = useCallback(
    async (device: CastDevice) => {
      ...
      try {
        status = await invoke<EmbeddedMpvStatus>('player_status');
      } catch {}
      ...
      const mediaUrl =
        initialStreamHeaders && Object.keys(initialStreamHeaders).length > 0
