...
export function usePlayerAnime4k({
  prefs,
  persistPreference,
  flashFeedback,
}: {
  ...
  useEffect(() => {
    ...
    void invoke<boolean>('player_get_anime4k_enabled')
      .then(setAnime4kEnabled)
      .catch(() => undefined);
    void listen<{ enabled?: boolean }>('player-anime4k-state', (event) => setAnime4kEnabled(event.payload.enabled === true)).then(
      (stop) => {
        unlisten = stop;
      },
    );
  ...
  const toggleAnime4k = useCallback(
    (enabled: boolean) => {
      ...
      void invoke('player_set_anime4k_enabled', {
        ...
        mode: anime4kMode,
      }).catch(() => setAnime4kEnabled(!enabled));
    },
    [anime4kMode, prefs],
  );

  const cycleAnime4kMode = useCallback(
    (direction: 1 | -1) => {
      ...
      void invoke('player_set_anime4k_enabled', { enabled: true, quality: String(prefs?.animeUpscalingQuality ?? 'anime4k_m'), mode: next })
        .then(() => {
          setAnime4kMode(next);
