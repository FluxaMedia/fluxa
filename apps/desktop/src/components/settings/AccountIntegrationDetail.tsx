...
export function AccountIntegrationDetail({
  ...
  const page =
    selectedIntegration === 'trakt'
      ? {
          ...
          meta: traktSyncMeta,
          error: traktError,
          connect: () => void handleTraktConnect(),
          sync: () => void handleTraktSyncNow(),
          disconnect: () => void handleTraktDisconnect(),
        }
      : selectedIntegration === 'anilist'
        ? {
            title: t('brand.anilist'),
            connected: anilistConnected,
            busy: anilistBusy,
            meta: anilistSyncMeta,
            error: anilistError,
            connect: () => void handleAnilistConnect(),
            sync: () => void handleAnilistSyncNow(),
            disconnect: () => void handleAnilistDisconnect(),
          }
        : selectedIntegration === 'simkl'
          ? {
              title: t('brand.simkl'),
              connected: simklConnected,
              busy: simklBusy,
              meta: simklSyncMeta,
              error: simklError,
              connect: () => void handleSimklConnect(),
              sync: () => void handleSimklSyncNow(),
              disconnect: () => void handleSimklDisconnect(),
            }
          : selectedIntegration === 'nuvio'
            ? {
                title: t('brand.nuvio'),
                connected: nuvioConnected,
                busy: nuvioBusy,
                meta: nuvioSyncMeta,
                error: nuvioError,
                connect: () => setNuvioFormOpen(true),
                sync: () => void handleNuvioSyncNow(),
                disconnect: () => void handleNuvioDisconnect(),
              }
            : {
                title: t('brand.stremio'),
                connected: stremioConnected,
                busy: stremioBusy,
  ...
  const oauthService =
    selectedIntegration === 'trakt' || selectedIntegration === 'anilist' || selectedIntegration === 'simkl' ? selectedIntegration : null;
  ...
  return (
    <>
      <div style={{ display: 'flex', alignItems: 'center' }}>
        ...
        <div style={{ flex: 1 }}>
          <SettingsDetailHeader title={page.title} />
        </div>
      </div>
      <SettingsSection title={t('settings.sync_with')} subtitle={t('settings.sync_with_desc')}>
        <SyncServiceRow
          icon={null}
          title={page.title}
          value={page.connected ? t('sync.device.connected') : t('settings.connect_account')}
          valueColor={page.connected ? '#54D17A' : undefined}
          onClick={page.connected ? undefined : page.connect}
          busy={page.busy}
        />
        {page.connected && (selectedIntegration === 'trakt' || selectedIntegration === 'simkl') && (
          <ProviderTokenStatus
            expiresAt={tokenExpiresAt}
            verified={Boolean(page.meta && !page.meta.error)}
            refreshScheduled={selectedIntegration === 'trakt'}
          />
        )}
        {!page.connected && oauthService && renderOAuthFallback(oauthService)}
        {!page.connected && selectedIntegration === 'nuvio' && nuvioFormOpen && (
          <CredentialLoginForm
            busy={nuvioBusy}
        ...
        {page.error && (
          <div style={{ padding: '0 1.125rem 0.625rem' }}>
            <p style={{ color: '#FF5D5D', fontSize: '0.75rem', margin: 0 }}>
              {t('common.error')}: {page.error}
            </p>
          </div>
        )}
      </SettingsSection>
      {page.connected && (
        <SettingsSection title={page.title} subtitle={t('settings.sync_with_desc')}>
          <SyncServiceRow
            icon={null}
            title={t('settings.sync_now')}
            value={page.meta ? new Date(page.meta.lastSyncAt).toLocaleString() : ''}
            onClick={page.sync}
            busy={page.busy}
          />
          <SyncServiceRow
            ...
            value=""
            onClick={() => setImportDialog(selectedIntegration)}
            busy={page.busy}
          />
          <SyncServiceRow
            icon={null}
            title={t('auto.disconnect')}
            value=""
            onClick={() => setConfirmDisconnect({ title: page.title, onConfirm: page.disconnect })}
            destructive
          />
        </SettingsSection>
      )}
      {page.connected && (
        <SettingsSection title={t('settings.provider_library')} subtitle={page.title}>
      ...
      {page.connected && selectedIntegration === 'trakt' && (
        <SettingsSection title={t('brand.trakt')} subtitle={t('settings.sync_with_desc')}>
          <ChoiceTile
            title={t('settings.continue_watching_window')}
            subtitle={t('settings.continue_watching_window_desc')}
            options={[
              { value: '0', label: t('settings.continue_watching_window_all') },
              { value: '7', label: '7' },
              { value: '30', label: '30' },
              { value: '90', label: '90' },
              { value: '365', label: '365' },
            ]}
            selected={prefs.continueWatchingDays}
            onSelect={(value) => setPref('continueWatchingDays', value)}
          />
          <ToggleTile
            title={t('settings.trakt_comments')}
            subtitle={t('settings.trakt_comments_desc')}
            checked={prefs.traktCommentsEnabled}
            onToggle={(value) => setPref('traktCommentsEnabled', value)}
          />
        </SettingsSection>
      )}
      {confirmDisconnect && (
      ...
      {importDialog && (
        <ImportDialog
          ...
          onScan={async (selected) => {
            ...
            const token =
              provider === 'trakt'
                ...
                : provider === 'simkl'
                  ...
                  : provider === 'anilist'
                    ? activeProfile.anilistAccessToken
                    : provider === 'stremio'
                      ? activeProfile.stremioAuthKey
                      : activeProfile.nuvioAccessToken;
            const clientId =
              provider === 'trakt'
                ? await platformInvoke<string>('get_oauth_client_id', { service: 'trakt' })
                : provider === 'simkl'
                  ? await platformInvoke<string>('get_oauth_client_id', { service: 'simkl' })
                  : undefined;
            ...
            if (selected.includes('watched'))
              counts.watched = result.watchedCount ?? (result.completedCount ?? 0) + (result.droppedCount ?? 0);
          ...
          onConfirm={(selected, destination) => {
            ...
            void (async () => {
              ...
              const traktClientId =
                destination === 'trakt' ? await platformInvoke<string>('get_oauth_client_id', { service: 'trakt' }) : undefined;
              const simklClientId =
                destination === 'simkl' ? await platformInvoke<string>('get_oauth_client_id', { service: 'simkl' }) : undefined;
