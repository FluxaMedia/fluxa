    ...
    async (
      ...
      try {
        ...
        const effectiveTotalDuration =
          ...
          (meta?.id
            ? (stateRef.current.library.lastWrite?.progress as Record<string, import('../core/types').LibraryItem> | undefined)?.[meta.id]
                ?.duration
            : undefined);
        lastTotalDurationSecondsRef.current = effectiveTotalDuration;

        const retryNextOrFail = async (message: string) => {
          const nextSource = await nextRetrySource(
            stream,
            message === t('player.torrent_no_peers') || message === t('player.torrent_too_slow'),
          );
        ...
        const nextEp = episode
          ? ((await coreResolveNextEpisode(
              JSON.stringify(meta?.videos ?? []),
              episode.season ?? 0,
              episode.episode ?? episode.number ?? 0,
              Date.now(),
              true,
            )) as Video | null)
          : null;
        ...
        debugLog(
          `handlePlay:plan ready mode=${playbackPlan?.mode} url=${(playbackPlan?.url ?? stream.playableUrl ?? stream.url)?.slice(0, 80)}`,
        );
        if (isCancelled()) return;
        ...
        if (playbackPlan?.mode === 'reject') {
          await retryNextOrFail(
            playbackPlan.rejectReason === 'incompatible_stream' ? t('player.incompatible_desktop_stream') : t('player.no_playable_url'),
          );
          return;
        }
        ...
        const subtitlesPromise = resolvePlaybackSubtitles(stream, meta, episode, playbackPlan?.subtitleExtraArgs, playbackAddons).catch(
          () => ({ subtitles: [], failedAddons: [] }) as ResolvedSubtitles,
        );
        ...
        debugLog(
          `handlePlay:anime detection confidence=${animeDetection.confidence} isAnime=${animeDetection.isAnime} reasons=${animeDetection.reasons.join(', ')}`,
        );
        ...
        const pollMpvLoadingStatus = async () => {
          while (loadingStatusPollActive && !isCancelled() && playerLoadingOverlayRef.current && !playerLoadingOverlayRef.current.error) {
            ...
            if (!loadingStatusPollActive || isCancelled() || !playerLoadingOverlayRef.current || playerLoadingOverlayRef.current.error)
              return;
            if (!status?.loaded) {
        ...
        if (playbackPlan?.mode === 'torrent') {
          ...
          try {
            ...
            for (let retryIndex = 0; retryIndex <= MAX_PEER_RETRIES; retryIndex++) {
              try {
                ...
                const torrentDurationMs =
                  typeof effectiveTotalDuration === 'number' && Number.isFinite(effectiveTotalDuration) && effectiveTotalDuration > 0
            ...
            await playInEmbeddedMpv(
              generation,
              localUrl,
              title,
              true,
              subtitlesPromise,
              loadingArtworkPromise,
              resumeAtSeconds,
              effectiveTotalDuration,
              undefined,
              animeDetection.isAnime,
            );
        ...
        void (async () => {
          try {
            ...
            if (fetchedVideos.length > 0 && episode) {
              resolvedNextEp = (await coreResolveNextEpisode(
            ...
            if (resolvedPlayableNextEp && (await coreCanPrefetchNextEpisode(JSON.stringify(prefs), JSON.stringify(stream)))) {
              void (async () => {
                try {
                  ...
                  if (streams.length > 0) {
                    const chosen = (await coreSelectNextEpisodeStream(
                      JSON.stringify(streams),
                      JSON.stringify(stream),
                      JSON.stringify(prefs),
                      resolvedPlayableNextEp.id,
                    )) as Stream | null;
