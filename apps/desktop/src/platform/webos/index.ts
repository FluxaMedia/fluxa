...
export interface MediaOptionHints {
  ...
  audioProcessingMode?: 'reference' | 'balanced' | 'night';
  adaptive?: boolean;
}

export function isBitstreamAudioCodec(codec: string | null | undefined): boolean {
  const value = `${codec ?? ''}`.toLowerCase().replace(/[._\s-]/g, '');
  return value.includes('truehd') ||
    value.includes('dolbymat') ||
    value.includes('eac3') ||
    value.includes('ac3') ||
    value.includes('ac4') ||
    value.includes('dts') ||
...
export async function applyWebOSMediaOption(video: HTMLVideoElement, url: string, hints: MediaOptionHints = {}): Promise<void> {
  ...
  const processingMode = hints.audioProcessingMode ?? 'reference';
  const requestedBitstream = hints.bitstreamAudio === true ||
    (hints.bitstreamAudio === undefined && isBitstreamAudioCodec(hints.audioCodec));
  const bitstreamRequired = processingMode === 'reference' &&
    requestedBitstream && capabilities?.dolbyAtmos === true;
  ...
  const option: Record<string, unknown> = {
    ...
    audioInfo: {
      ...
      isBitstreamRequired: bitstreamRequired,
    },
  };
