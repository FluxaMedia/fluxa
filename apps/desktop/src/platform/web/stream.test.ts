...
describe('choosePlaybackUrl in a browser', () => {
  it('maps compressed audio codecs to valid MP4 codec identifiers', async () => {
    await withTarget('web', ({ codecString }) => {
      expect(codecString('hevc', 'eac3')).toContain('ec-3');
      expect(codecString('hevc', 'E-AC-3')).toContain('ec-3');
      expect(codecString('h264', 'ac3')).toContain('ac-3');
      expect(codecString('h264', 'flac')).toContain('flac');
    });
  });
