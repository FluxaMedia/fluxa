...
#[tauri::command]
pub fn library_progress_upsert_many(
    state: State<DesktopState>,
    profile_key: String,
    updates_json: String,
) -> bool {
    ...
    let profile_key = sanitize_key(&profile_key);
    let Ok(mut database) = open_database(&dir) else {
        return false;
    };
    if ensure_progress_migrated(&database, &dir, &profile_key).is_err() {
        return false;
    }
    let Ok(updates) = serde_json::from_str::<Vec<Value>>(&updates_json) else {
        return false;
    };
    let Ok(transaction) = database.transaction() else {
        return false;
    };
    for update in updates {
        let Some(media_id) = update.get("mediaId").and_then(Value::as_str) else {
            return false;
        };
        let Some(value) = update.get("value") else {
            return false;
        };
        let Ok(value) = serde_json::to_vec(value).and_then(|value| {
            encrypt(&dir, &value).map_err(|e| serde_json::Error::io(std::io::Error::other(e)))
