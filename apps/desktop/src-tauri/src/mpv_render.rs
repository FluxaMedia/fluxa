...
struct MpvAudioPolicyState {
    passthrough: bool,
    dsp: bool,
    fallback_attempted: bool,
}

fn audio_output_mode_for_policy(policy: MpvAudioPolicyState) -> &'static str {
    if policy.dsp {
        "pcm_dsp"
    } else if policy.fallback_attempted {
        "pcm_fallback"
...
pub struct MpvClientHandle {
    ...
    audio_policy: Mutex<MpvAudioPolicyState>,
...
impl PlayerStatus {
    pub fn static_status(&self) -> PlayerStaticStatus {
        PlayerStaticStatus {
            ...
            audio_output_mode: self.audio_output_mode.clone(),
