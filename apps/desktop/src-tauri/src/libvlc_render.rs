...
fn audio_policy_state_from_options(
    current: AudioPolicyState,
    options: &[(String, String)],
) -> AudioPolicyState {
    let mut next = current;
    for (name, value) in options {
        match name.as_str() {
            "audio-spdif" => next.passthrough = !value.trim().is_empty(),
            "af" => next.dsp = !value.trim().is_empty(),
            _ => {}
        }
    }
    // A DSP chain needs decoded PCM. Never allow a stale passthrough flag to
    // survive a mode change, even if the caller supplied the options in a
    // different order.
    if next.dsp {
        next.passthrough = false;
    }
    next
}
...
pub struct LibvlcPlayer {
    ...
    audio_passthrough: Mutex<bool>,
    audio_dsp: Mutex<bool>,
    audio_fallback_attempted: Mutex<bool>,
    loaded: bool,
}
...
impl LibvlcPlayer {
    pub fn new() -> Result<Self, String> {
        ...
        Ok(Self {
            ...
            // Reference is the safe default. LibVLC itself still decides
            // whether the active audio output can actually open SPDIF.
            audio_passthrough: Mutex::new(true),
            audio_dsp: Mutex::new(false),
            audio_fallback_attempted: Mutex::new(false),
            loaded: false,
        })
    }
...
impl PlaybackEngine for LibvlcPlayer {
    fn load(&mut self, url: &str, start_at: Option<u64>) -> Result<(), String> {
        ...
        if self.current_url.as_deref() != Some(url) {
            *self.audio_fallback_attempted.lock().unwrap() = false;
        }
        ...
        let audio_passthrough = *self.audio_passthrough.lock().unwrap();
        let audio_dsp = *self.audio_dsp.lock().unwrap();
        if audio_passthrough && !audio_dsp {
            let opt = CString::new(":spdif").expect("static VLC option");
            unsafe { (self.api.media_add_option)(media, opt.as_ptr()) };
        } else if audio_dsp {
            // VLC's compressor is the portable PCM fallback. It is less
            // expressive than the MPV/Android float DSP, but importantly it
            // cannot be applied to a passthrough bitstream.
            let opt = CString::new(":audio-filter=compressor").expect("static VLC option");
            unsafe { (self.api.media_add_option)(media, opt.as_ptr()) };
        }
    ...
    fn apply_options(&self, options: &[(String, String)]) -> Result<(), String> {
        let current = AudioPolicyState {
            passthrough: *self.audio_passthrough.lock().unwrap(),
            dsp: *self.audio_dsp.lock().unwrap(),
        };
        let next = audio_policy_state_from_options(current, options);
        if next.passthrough && !current.passthrough {
            *self.audio_fallback_attempted.lock().unwrap() = false;
        }
        *self.audio_passthrough.lock().unwrap() = next.passthrough;
        *self.audio_dsp.lock().unwrap() = next.dsp;
        Ok(())
    }
    ...
    fn status(&self) -> PlayerStatus {
        ...
        let audio_output_mode = if *self.audio_dsp.lock().unwrap() {
            "pcm_dsp"
        } else if *self.audio_fallback_attempted.lock().unwrap() {
            "pcm_fallback"
        } else if *self.audio_passthrough.lock().unwrap() {
            "passthrough"
        } else {
            "pcm"
        };
        ...
        PlayerStatus {
            ...
            audio_channels: None,
            audio_output_mode: Some(audio_output_mode.to_string()),
    ...
    fn poll_events(&mut self) -> Vec<PlayerEvent> {
        ...
        let events: Vec<PlayerEvent> = queue.events.drain(..).collect();
        drop(queue);

        let encountered_error = events.iter().any(|event| {
            matches!(
                event,
                PlayerEvent::EndFile {
                    eof: false,
        ...
        let passthrough = *self.audio_passthrough.lock().unwrap();
        let dsp = *self.audio_dsp.lock().unwrap();
        let fallback_attempted = *self.audio_fallback_attempted.lock().unwrap();
        if encountered_error && passthrough && !dsp && !fallback_attempted {
            let url = self.current_url.clone();
            if let Some(url) = url {
                let position_ms = if self.media_player.is_null() {
                    0
                } else {
                    unsafe { (self.api.media_player_get_time)(self.media_player) }.max(0) as u64
                };
                *self.audio_fallback_attempted.lock().unwrap() = true;
                *self.audio_passthrough.lock().unwrap() = false;
                log::warn!(
                    "LibVLC playback error while passthrough was active; retrying audio as PCM at {}ms",
                    position_ms
                );
                if self.load(&url, Some(position_ms)).is_ok() {
                    // The first error was recovered transparently. Do not let
                    // the UI close the player while the PCM retry is starting.
                    return Vec::new();
                }
                log::warn!("LibVLC PCM retry failed; forwarding the original playback error");
            }
        }
        events
    }
...
mod tests {
    ...
    #[test]
    fn dsp_always_disables_passthrough() {
        let state = audio_policy_state_from_options(
            AudioPolicyState {
                passthrough: true,
                dsp: false,
            },
            &options(&[("af", "lavfi=[acompressor]")]),
        );
        assert_eq!(
            state,
            AudioPolicyState {
    ...
    #[test]
    fn reference_options_clear_stale_dsp() {
        let state = audio_policy_state_from_options(
            AudioPolicyState {
                passthrough: false,
                dsp: true,
            },
            &options(&[("audio-spdif", "ac3,eac3"), ("af", "")]),
        );
        assert_eq!(
            state,
            AudioPolicyState {
