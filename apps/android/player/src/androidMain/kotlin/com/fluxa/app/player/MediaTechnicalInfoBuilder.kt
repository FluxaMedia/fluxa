...
internal object MediaTechnicalInfoBuilder {
    fun build(
        ...
        audioOffloadActive: Boolean = false,
        audioSinkStatus: String? = null,
    ): String {
        ...
        val audioCapabilities = AudioCapabilityResolver.resolve(context, audioAttributes)
        val audioPlan = audioFormat?.let { AudioCapabilityResolver.plan(context, it, audioAttributes) }
        val audioPath = when {
            audioProcessingMode == "night" -> "pcm_dsp"
            audioProcessingMode == "balanced" && audioPlan != null -> "pcm_gentle_dsp"
            audioOffloadActive -> "offload"
            exoPlayer.isSleepingForOffload -> "offload_sleeping"
            audioPlan?.mode == AudioOutputMode.PASSTHROUGH -> "passthrough"
            audioPlan?.mode == AudioOutputMode.DOWNMIX -> "pcm_downmix"
            audioPlan != null -> "pcm"
            else -> "unknown"
        }
        val configuredMime = audioSinkStatus
            ?.substringAfter("mime=", "")
            ?.substringBefore(' ')
            ?.lowercase()
            .orEmpty()
        val observedAudioPath = when {
            audioProcessingMode != "reference" -> "pcm_dsp"
            audioOffloadActive -> "offload"
            exoPlayer.isSleepingForOffload -> "offload_sleeping"
            configuredMime == "audio/raw" || configuredMime == "audio/pcm" -> "pcm_sink_configured"
            configuredMime.startsWith("audio/") -> "encoded_sink_configured"
            audioDecoderName?.contains("ffmpeg", ignoreCase = true) == true -> "pcm_ffmpeg"
            audioDecoderName != null -> "pcm_decoder"
            else -> "pending"
        }
        return listOf(
            ...
            "audio_decoder_actual=${audioDecoderName ?: "unknown"}",
            "audio_processing=$audioProcessingMode",
            "audio_path=$audioPath",
            "audio_path_observed=$observedAudioPath",
            "audio_output=${audioCapabilities.routeDescription} pcm_channels=${audioCapabilities.maxPcmChannels} pcm_sample_rates=${audioCapabilities.pcmSampleRates} passthrough=${audioCapabilities.passthroughEncodings}",
            audioSinkStatus?.let { "audio_sink_configured=$it" },
            audioPlan?.let {
                val processingForcesPcm = audioProcessingMode != "reference" && it.mode == AudioOutputMode.PASSTHROUGH
                val reportedMode = if (processingForcesPcm) AudioOutputMode.PCM else it.mode
                val reason = if (processingForcesPcm) {
                    "$audioProcessingMode mode forces decoded PCM"
                } else {
                    it.reason
                }
                "audio_plan=$reportedMode $reason source=${it.sourceMimeType} ${it.sourceChannels}ch ${it.sourceSampleRate}Hz"
            },
            "ffmpeg=$ffmpeg",
            "buffer=$bufferHealth",
            color,
