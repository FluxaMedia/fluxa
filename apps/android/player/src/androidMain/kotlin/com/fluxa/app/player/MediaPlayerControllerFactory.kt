...
internal object MediaPlayerControllerFactory {
    ...
    private val audioProcessingModes = Collections.synchronizedMap(WeakHashMap<ExoPlayer, String>())
    private val audioOffloadStates = Collections.synchronizedMap(WeakHashMap<ExoPlayer, Boolean>())
    private val audioFallbackRetryStates = Collections.synchronizedMap(WeakHashMap<ExoPlayer, Boolean>())
    private val audioRendererFactories = Collections.synchronizedMap(WeakHashMap<ExoPlayer, AppAudioRendererFactory>())
    private val audioRouteRegistrations = Collections.synchronizedMap(
        WeakHashMap<ExoPlayer, Pair<android.media.AudioManager, android.media.AudioDeviceCallback>>()
    )
    private val audioPlaybackRegistrations = Collections.synchronizedMap(
        WeakHashMap<ExoPlayer, Pair<android.media.AudioManager, android.media.AudioManager.AudioPlaybackCallback>>()
    )
    private val audioSpatializerRegistrations = Collections.synchronizedMap(
        WeakHashMap<ExoPlayer, Pair<android.media.Spatializer, android.media.Spatializer.OnSpatializerStateChangedListener>>()
    )
    private val audioRouteRefreshPending = Collections.synchronizedMap(WeakHashMap<ExoPlayer, Boolean>())
    private val audioRouteListeners = Collections.synchronizedMap(
        WeakHashMap<ExoPlayer, () -> Unit>()
    )
    ...
    internal fun setAudioFallbackRetrying(player: ExoPlayer, retrying: Boolean) {
        if (retrying) audioFallbackRetryStates[player] = true else audioFallbackRetryStates.remove(player)
    }
    internal fun audioSinkRuntimeStatus(player: ExoPlayer): String? =
        audioRendererFactories[player]?.audioSinkRuntimeStatus()
    ...
    fun releaseExoPlayer(player: ExoPlayer) {
        ...
        audioProcessingModes.remove(player)
        audioOffloadStates.remove(player)
        audioFallbackRetryStates.remove(player)
        audioRendererFactories.remove(player)
        audioRouteRegistrations.remove(player)?.let { (manager, callback) ->
            if (android.os.Build.VERSION.SDK_INT >= 23) {
                runCatching { manager.unregisterAudioDeviceCallback(callback) }
            }
        }
        audioPlaybackRegistrations.remove(player)?.let { (manager, callback) ->
            if (android.os.Build.VERSION.SDK_INT >= 24) {
                runCatching { manager.unregisterAudioPlaybackCallback(callback) }
            }
        }
        audioSpatializerRegistrations.remove(player)?.let { (spatializer, listener) ->
            if (android.os.Build.VERSION.SDK_INT >= 31) {
                runCatching { spatializer.removeOnSpatializerStateChangedListener(listener) }
            }
        }
        audioRouteRefreshPending.remove(player)
        audioRouteListeners.remove(player)
        requestContexts.remove(player)
        runCatching { player.release() }
    }

    internal fun setAudioRouteListener(player: ExoPlayer, listener: (() -> Unit)?) {
        if (listener == null) audioRouteListeners.remove(player) else audioRouteListeners[player] = listener
    }

    internal fun blacklistAudioCodec(player: ExoPlayer, codecName: String): Boolean =
        audioRendererFactories[player]?.blacklistAudioCodec(codecName) == true

    internal fun blacklistAudioMimeType(player: ExoPlayer, mimeType: String?): Boolean =
        audioRendererFactories[player]?.blacklistAudioMimeType(mimeType) == true

    internal fun resetAudioCodecFailures(player: ExoPlayer) {
        audioRendererFactories[player]?.resetAudioCodecFailures()
    }

    /**
     * Re-prepares the retained media item at the current position. The failed
     * MediaCodec is already blacklisted, so the next renderer pass reaches the
     * FFmpeg extension renderer without replacing the player object used by UI.
     */
    internal fun retryAudioWithFfmpeg(player: ExoPlayer): Boolean {
        val factory = audioRendererFactories[player] ?: return false
        if (!AudioDecoderSupport.isFfmpegAvailable()) return false
    ...
    private fun registerAudioRouteCallback(player: ExoPlayer, context: Context) {
        ...
        val callback = object : android.media.AudioDeviceCallback() {
            override fun onAudioDevicesAdded(addedDevices: Array<out android.media.AudioDeviceInfo>) {
                scheduleAudioRouteRefresh(player)
            }
        ...
        manager.registerAudioDeviceCallback(callback, android.os.Handler(android.os.Looper.getMainLooper()))
        audioRouteRegistrations[player] = manager to callback
        if (android.os.Build.VERSION.SDK_INT >= 24) {
            // A route can change between already-connected devices (for
            // example speaker -> Bluetooth or one HDMI sink -> another)
            // without an add/remove callback. Playback configuration changes
            // are the closest public signal for that active-output switch.
            val playbackCallback = object : android.media.AudioManager.AudioPlaybackCallback() {
            ...
            audioPlaybackRegistrations[player] = manager to playbackCallback
        }
        if (android.os.Build.VERSION.SDK_INT >= 31) {
            ...
            audioSpatializerRegistrations[player] = spatializer to listener
        }
    }

    private fun scheduleAudioRouteRefresh(player: ExoPlayer) {
        synchronized(audioRouteRefreshPending) {
            if (audioRouteRefreshPending[player] == true) return
            audioRouteRefreshPending[player] = true
        }
        android.os.Handler(android.os.Looper.getMainLooper()).postDelayed({
            audioRouteRefreshPending.remove(player)
            refreshAudioRoute(player)
        }, 150L)
    }

    private fun refreshAudioRoute(player: ExoPlayer) {
        val factory = audioRendererFactories[player] ?: return
        if (player.mediaItemCount == 0) return
        val positionMs = player.currentPosition.coerceAtLeast(0L)
        val shouldPlay = player.playWhenReady
        runCatching {
            factory.resetAudioCodecFailures()
            if (!factory.refreshAudioRoute()) {
                // The callback can describe an unrelated connected device
                // rather than a change to the active media route. Avoid
                // interrupting playback in that case.
                return@runCatching
            }
            player.stop()
            audioRouteListeners[player]?.invoke()
            player.seekTo(positionMs)
            player.prepare()
            player.playWhenReady = shouldPlay
        }
    ...
    fun createExoPlayer(
        ...
        val safeAudioProcessingMode = when (audioProcessingMode) {
            "balanced", "night" -> audioProcessingMode
            else -> "reference"
        }
        val renderersFactory = AppAudioRendererFactory(context, audioDecoderMode, safeAudioProcessingMode, subtitleCoordinator).apply {
            // Keep PCM processing in float where the platform sink supports it. This does not
            // alter passthrough streams; encoded output still takes the sink's direct path.
        ...
        val trackSelector = DefaultTrackSelector(context).apply {
            parameters = buildUponParameters()
                ...
                .apply {
                    val offloadMode = if (safeAudioProcessingMode != "reference" || audioDecoderMode == "sw_only") {
        ...
        player.addAudioOffloadListener(object : ExoPlayer.AudioOffloadListener {
            override fun onOffloadedPlayback(isOffloadedPlayback: Boolean) {
                audioOffloadStates[player] = isOffloadedPlayback
            }
        })
        return player.apply {
            playWhenReady = true
            ...
            audioProcessingModes[this] = safeAudioProcessingMode
            audioRendererFactories[this] = renderersFactory
            registerAudioRouteCallback(this, context)
