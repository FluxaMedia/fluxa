    ...
    DisposableEffect(exoPlayer, useMpvBackend) {
        ...
        val listener = object : Player.Listener {
            ...
            override fun onPlayerError(error: androidx.media3.common.PlaybackException) {
                ...
                if (com.fluxa.app.player.MediaPlayerController.isAudioFallbackRetrying(exoPlayer)) {
...
internal fun PlayerPreparePlaybackEffect(
    ...
    lastSavedPlayWhenReady: Boolean?,
    ...
    // The engine is recreated when the audio route changes. Re-keying here is
    // required so the same media item is prepared on the new capability-aware
    // sink instead of leaving the replacement player idle.
    LaunchedEffect(resolvedUrl, activeEngine) {
        ...
        if (!playbackUrl.isNullOrEmpty()) {
            ...
            activeEngine?.prepareAndPlay(
                PlayerEngineRequest(
                    ...
                    shouldPlay = lastSavedPlayWhenReady ?: true
                )
            )
            lastSavedPlayWhenReady?.let { shouldPlay ->
                activeEngine?.setPaused(!shouldPlay)
            }
