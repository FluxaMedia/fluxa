        ...
        fun applyCoreState(state: PlayerRuntimeCoreState) {
            ...
            updateEngine {
                copy(
                    ...
                    playerError = localizedPlayerError(lang, state.playerError)
                )
            }
        }
...
internal fun localizedPlayerError(lang: String, rawError: String?): String? = when (rawError) {
    null -> null
    "no_source" -> AppStrings.t(lang, "player.error_no_source")
    else -> AppStrings.format(lang, "player.error_generic", rawError)
}
