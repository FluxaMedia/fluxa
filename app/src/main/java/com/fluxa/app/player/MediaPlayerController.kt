...
class MediaPlayerController(internal val context: Context, val exoPlayer: ExoPlayer) {
    ...
    private val _activeVideoFormatBadge = MutableStateFlow<VideoFormatBadge?>(null)
    val activeVideoFormatBadge: StateFlow<VideoFormatBadge?> = _activeVideoFormatBadge
    ...
    companion object {
        private class ExoRequestContext {
            ...
            // Set by: manifest rewrite (HLS P5), container DVCC interceptor (OkHttp MKV/MP4),
            // or Rust auto_detect proxy (auth-header MKV/MP4).
            @Volatile var needsIptPqc2ToneMap: Boolean = false
            // Cached once per playback session — MediaCodecList scan is expensive and the
            // codec list never changes mid-session.
            @Volatile var cachedCapabilities: DolbyVisionCapabilities? = null
            // DV8.1 conversion settings for inline HLS segment rewriting.
        ...
        private class SelectiveCacheDataSource(
            ...
            override fun open(dataSpec: DataSpec): Long {
                val selected = if (shouldUsePlayerDiskCache(dataSpec.uri)) {
        ...
        fun createExoPlayer(
            ...
            val dataSourceFactory = DataSource.Factory {
                SelectiveCacheDataSource(cachedDataSourceFactory, uncachedDataSourceFactory)
            }
            ...
            return ExoPlayer.Builder(context, renderersFactory)
                ...
                .setVideoChangeFrameRateStrategy(androidx.media3.common.C.VIDEO_CHANGE_FRAME_RATE_STRATEGY_OFF)
                .setAudioAttributes(
                ...
                .build().apply {
                    playWhenReady = true
                    // Prevents Android power manager from throttling CPU/network during playback,
                    // which causes periodic video frame drops on battery-optimized devices.
        ...
        private fun rewriteDolbyVisionManifestResponse(
            ...
            if (!isManifest) {
                ...
                if (!isProxy) {
                    // HLS fMP4 / TS segment RPU conversion for ConvertToDv81 mode.
                    // The manifest rewrite changed the codec string to dvhe.08; here we convert
                    // the actual per-segment RPU NALs so the DV8.1 decoder receives correct data.
                    if (mode == DolbyVisionFallbackMode.ConvertToDv81) {
                        ...
                        if (caps.mediaCodecSupportsDolbyVision && !caps.displaySupportsDolbyVision) {
        ...
        private fun maybeStripContainerDvcc(
            ...
            val deviceSupportsDv = caps.mediaCodecSupportsDolbyVision &&
                (caps.displaySupportsDolbyVision || dvInfo.notHasHdrFallback)

            if (!deviceSupportsDv && (dvInfo.profile == 5 || !dvInfo.notHasHdrFallback)) {
                DolbyVisionFallbackPolicy.mangleDvccFourcc(headerBytes)
                if (dvInfo.profile == 5 && dvInfo.compatId != 1) {
                    requestContext.needsIptPqc2ToneMap = true
                }
            }
            return reconstructResponse(response, body, headerBytes, source)
        }
    ...
    private fun updateTracks(tracks: Tracks) {
            ...
            if (group.type == C.TRACK_TYPE_AUDIO) {
                for (i in 0 until group.length) {
                    ...
                    audios.add(MediaTrack(
                        ...
                        showCodecBadge = group.isTrackSupported(i) &&
                            MediaPlaybackBadgeSupport.supportsEncodedAudioBadge(context, format.sampleMimeType),
        ...
        var videoBadge: VideoFormatBadge? = null
        for (group in tracks.groups) {
            ...
            for (i in 0 until group.length) {
                ...
                videoBadge = when {
                    fmt.sampleMimeType == MimeTypes.VIDEO_DOLBY_VISION &&
                        MediaPlaybackBadgeSupport.supportsHdrBadge(context, VideoFormatBadge.DolbyVision) -> VideoFormatBadge.DolbyVision
                    fmt.colorInfo?.colorTransfer == C.COLOR_TRANSFER_HLG &&
                        MediaPlaybackBadgeSupport.supportsHdrBadge(context, VideoFormatBadge.Hlg) -> VideoFormatBadge.Hlg
                    fmt.colorInfo?.colorTransfer == C.COLOR_TRANSFER_ST2084 &&
                        MediaPlaybackBadgeSupport.supportsHdrBadge(context, VideoFormatBadge.Hdr10) -> VideoFormatBadge.Hdr10
                    else -> null
                }
                break
            }
            if (videoBadge != null) break
        }
        _activeVideoFormatBadge.value = videoBadge
    }
    ...
    fun prepareAndPlay(
        ...
        requestContexts[exoPlayer]?.let { context ->
            context.streamHeaders = headers.orEmpty()
            context.dolbyVisionFallbackMode = dolbyVisionFallbackMode
            context.lastDolbyVisionDecision = when (dolbyVisionFallbackMode) {
                DolbyVisionFallbackMode.Off -> "dv_fallback=off"
                else -> "dv_fallback=pending_manifest_check"
            }
            context.needsIptPqc2ToneMap = false
            context.cachedCapabilities = null
            context.dvRpuMode = dvRpuMode
            context.dvZeroLevel5 = dvZeroLevel5
            context.dvHdr10PlusMode = dvHdr10PlusMode
        }
        val builder = MediaItem.Builder().setUri(Uri.parse(url))
        val lowerUrl = url.lowercase()
        if (lowerUrl.contains("m3u8")) builder.setMimeType(MimeTypes.APPLICATION_M3U8)
