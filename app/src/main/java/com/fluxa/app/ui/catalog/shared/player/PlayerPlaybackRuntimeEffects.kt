...
internal fun PlayerResolveUrlEffect(
    ...
    LaunchedEffect(currentUrl) {
        ...
        if (resolved.playerError == null) {
            ...
            updateEngine { copy(playerError = resolved.playerError, playback = playback.copy(isBuffering = false)) }
    ...
    LaunchedEffect(resolvedUrl) {
        ...
        if (!playbackUrl.isNullOrEmpty()) {
            ...
            val dolbyVisionFallbackMode = when (activeProfile?.safeDolbyVisionFallbackMode) {
                ...
                "convert_dv81" -> DolbyVisionFallbackMode.ConvertToDv81
            ...
            launch(Dispatchers.IO) {
                val externalSubtitles = fetchExternalSubtitleTracks(
                    viewModel = viewModel,
                ...
                if (externalSubtitles.isNotEmpty()) {
                    launch(Dispatchers.Main) {
                        activeEngine?.addExternalSubtitles(externalSubtitles)
                        onExternalSubtitlesFetched(externalSubtitles)
                    }
                }
            }
