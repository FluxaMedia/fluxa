...
internal fun HomeHeroBackdrop(
    ...
    val ambient = rememberAmbientHeroColor(selectedContent?.homeHeroBackdrop(seasonPostersOnHero))
    Box(modifier = Modifier.fillMaxSize()) {
        when {
            ...
            activePreviewUrl?.isNotBlank() == true -> {
                ...
                val backdropUrl = selectedContent?.homeHeroBackdrop(seasonPostersOnHero)
                val backdropRequest = remember(heroContext, backdropUrl) {
                    backdropUrl?.let {
                        coil3.request.ImageRequest.Builder(heroContext)
                            .data(it)
                            .memoryCacheKey(it)
                            .diskCacheKey(it)
            ...
            modifier = Modifier
                ...
                .background(
                    Brush.verticalGradient(
                        ...
                        1f to ambient.copy(alpha = 0.42f)
                    )
                )
        )
    }
...
internal fun HomeHeroPanel(
    ...
    aboveHeroFocusRequester: FocusRequester? = null,
    modifier: Modifier = Modifier,
    onPlayClick: () -> Unit,
    onInfoClick: () -> Unit
) {
        ...
        Row(
            ...
            Button(
                ...
                modifier = Modifier
                    ...
                    .then(
                        if (aboveHeroFocusRequester != null) Modifier.focusProperties { up = aboveHeroFocusRequester }
                        else Modifier
                    )
                    .onFocusChanged { if (it.isFocused) onHeroFocused?.invoke() }
                    .onKeyEvent { event ->
                        if (event.type == KeyEventType.KeyDown && event.key == Key.DirectionDown && onNavigateDown != null) {
                            onNavigateDown()
                            true
                        } else false
                    },
