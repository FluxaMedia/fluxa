...
object FluxaCoreNative {
    ...
    internal fun parseHeadlessResult(json: String): NativeHeadlessEngineResult {
    ...
    fun findPreferredSubtitleIndex(
        availableSubtitles: List<MediaTrack>,
        lastSubtitleLanguage: String?,
        preferredSubtitleLanguage: String?,
        secondarySubtitleLanguage: String?
    ): Int = call {
        val tracks = availableSubtitles.map { track ->
            NativeSubtitleSelectionTrack(
                id = track.id,
                label = track.label,
                language = track.language
            )
        }
        findPreferredSubtitleIndexNative(
            gson.toJson(tracks),
            lastSubtitleLanguage.orEmpty(),
            preferredSubtitleLanguage.orEmpty(),
            secondarySubtitleLanguage.orEmpty()
        )
    }
    ...
    fun playerTrackState(
        ...
        val json = playerTrackStateJsonNative(
            gson.toJson(
                mapOf(
                    "availableSubtitles" to tracks,
                    ...
                    "secondarySubtitleLanguage" to secondarySubtitleLanguage
                )
            )
        )
        gson.fromJson(json, NativePlayerTrackState::class.java) ?: NativePlayerTrackState()
    }
    ...
    fun contentTraktKey(meta: Meta): String = call {
        contentTraktKeyNative(gson.toJson(meta))
    }

    fun contentBillboardKey(meta: Meta): String = call {
        contentBillboardKeyNative(gson.toJson(meta))
    }
    ...
    fun contentWatchedKeys(meta: Meta): Set<String> = call {
        val json = contentWatchedKeysJsonNative(gson.toJson(meta))
        (gson.fromJson<List<String>>(json, stringListType) ?: emptyList()).toSet()
    }
    ...
    fun billboardScoreCandidate(meta: Meta, daysSinceRelease: Long?): Int = call {
        billboardScoreCandidateNative(
            gson.toJson(meta),
            daysSinceRelease ?: 0L,
    ...
    fun billboardHasBackdropCandidate(meta: Meta): Boolean = call {
        billboardHasBackdropCandidateNative(gson.toJson(meta))
    }

    fun billboardVisualScore(meta: Meta): Int = call {
        billboardVisualScoreNative(gson.toJson(meta))
    }

    fun billboardEditorialMatchScore(meta: Meta, spec: Any): Int = call {
        billboardEditorialMatchScoreNative(gson.toJson(meta), gson.toJson(spec))
    }
    ...
    private external fun contentTraktKeyNative(metaJson: String): String
    private external fun contentBillboardKeyNative(metaJson: String): String
    private external fun contentMergeKeysJsonNative(metaJson: String): String
    private external fun contentWatchedKeysJsonNative(metaJson: String): String
    ...
    private external fun billboardScoreCandidateNative(
        metaJson: String,
        daysSinceRelease: Long,
        hasDaysSinceRelease: Boolean
    ): Int
    private external fun billboardHasBackdropCandidateNative(metaJson: String): Boolean
    private external fun billboardVisualScoreNative(metaJson: String): Int
    private external fun billboardEditorialMatchScoreNative(metaJson: String, specJson: String): Int
